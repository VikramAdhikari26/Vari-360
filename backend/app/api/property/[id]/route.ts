...
export async function GET(
  ...
  try {
    ...
    const previousTax = Number(property.previous_tax);
    const currentTax = Number(property.current_tax);

    const previousArea = Number(property.previous_area);
    const currentArea = Number(property.current_area);

    const taxDifference = currentTax - previousTax;

    const taxPercentageChange =
      previousTax > 0
        ? (taxDifference / previousTax) * 100
        : 0;

    const areaDifference = currentArea - previousArea;

    const areaPercentageChange =
      previousArea > 0
        ? (areaDifference / previousArea) * 100
        : 0;
    ...
    if (taxPercentageChange > 10) {
      ...
      status = "Reduced";
      primaryReason =
        "The current assessment shows a reduction in property tax.";
    }
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        area: {
          ...
          percentageChange: Number(
            areaPercentageChange.toFixed(2)
          ),
        },
